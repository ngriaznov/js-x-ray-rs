...
- **749/749 etalon cases.** A corpus extracted from upstream's entire test
  ...
  *actual* `@nodesecure/js-x-ray` (warnings with locations, flags, scores,
  dependencies). `cargo test -p js-x-ray-rs --test etalon` replays it.
- **~470 ported unit tests** covering the internal APIs (walker, estree
...
```rust
use js_x_ray_rs::{AstAnalyser, AstAnalyserOptions, OptionalWarnings, RuntimeOptions, Sensitivity};
